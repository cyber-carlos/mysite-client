<template>
  <div v-if="$route.path !== '/'" class="flex px-8 pt-3.5 text-sm dark:text-saoqi-text2">
    <div v-for="(item, index) in items" :key="index">
      <IconHome v-if="index === 0" class="inline-block w-3 h-3 mr-1" />
      <IconChevronRight v-else-if="index < items.length && item.meta.label" class="inline-block w-3 h-3 ml-1" />
      <router-link :to="item.path" class="dark:hover:text-saoqi-text5">{{ item.meta.label }}</router-link>
    </div>
    <div class="ml-auto">
      <a
        href="#"
        class="align-middle hover:cursor-pointer dark:hover:text-saoqi-lighter2"
